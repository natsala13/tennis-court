...
def create_list_of_days(preferences: dict[str, dict[str, list[int]]]) -> dict[dict[int, set[int]]]:
    all_days = {day for category in preferences.values() for day in category['days']}
    all_days_cartezian = [(day, category['hours'], category['courts'])
                            for category in preferences.values()
                                for day in category['days']]

    option_per_day = {day: [options 
                      for day_option, *options in all_days_cartezian
                       if day_option == day]
                    for day in all_days}
    return {
        day: dict((court, {x for x in config[0]})
                      for config in days_configs for court in config[-1])
...
def find_courts_on_preference():
    ...
    for day in next_days:
        ...
        result = clubs.json()
        result_per_club = {club['club_id']: club['availableSlots'] for club in result['clubs']}
        slots_per_club = {club: parse_free_hours(slots) for club, slots in result_per_club.items()}
        # hours = parse_free_hours(result['clubs'][0]['availableSlots'])
        wanted_slots = {court: hours & set(slots_per_club[court]) for court, hours in spots.items()}
        existing_slots = {court: hours for court, hours in wanted_slots.items() if hours}
        
        if existing_slots:
            for court, hours in existing_slots.items():
                print(f'{DAYS[(day.weekday() + 1) % 7 + 1]} - {day.strftime("%d / %m")} - {COURTS[court]} - {hours}')
